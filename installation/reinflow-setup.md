...
This document records all the steps needed to install packages related to `DM1: MeanFlow with Dispersive Regularization for 1-Step Robotic Manipulation`.

**Note**: This is a pretraining-only release. The environment name has been changed to `dm1` instead of `reinflow`. 
...
conda activate dm1
# rename your repo folder as dm1-release
# (omitted)
```
...
cd <PATH_TO_YOUR_ANACONDA3>/envs/dm1/bin/../lib/ # 2. then create soft links
...
python <PATH_TO_YOUR_ANACONDA3>/envs/dm1/lib/python3.8/site-packages/robosuite/scripts/setup_macros.py # add links
# then you will see this "copied $HOME/anaconda3/envs/dm1/lib/python3.8/site-packages/robosuite/macros.py to $HOME/anaconda3/envs/dm1/lib/python3.8/site-packages/robosuite/macros_private.py"
```
...
If you want to obtain the training record of our experiments, consider download the visualize folder from our repository and put the contents in your `<DM1_DIR>/visualize`
...
* cannot find /util
This mean you have not yet installed the dm1 package. Please return to `7. Install DM1 package`. 
If you meet problems installing this package, or refuse to install it, we also provide a workaround: insert the following code before your [script/run.py](/script/run.py): 
```python
import os
import sys
